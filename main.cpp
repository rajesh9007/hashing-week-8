...
const unsigned int functions::hash_keys[64] = {
...
void functions::compress(const unsigned char *message,
...
void functions::adjust_digest(const unsigned char *text,
...
void functions::digest_final(unsigned char *digest)
{
    unsigned int block_nb;
    unsigned int pm_len;
    unsigned int len_b;
...
{
    unsigned char digest[functions::PADD_SIZE];
    memset(digest, 0, functions::PADD_SIZE);
    ...
    char buf[2 * functions::PADD_SIZE + 1];
    buf[2 * functions::PADD_SIZE] = 0;
    for (int i = 0; i < functions::PADD_SIZE; i++)
